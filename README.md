...
当一个正在运行的线程A，当遇到了`suspend`创建的协程，这个函数就会告知当前线程A“别管我，我自己完全”。然后当前线程A继续直行接下来的任务。这时协程就挂起了。那么函数代码块就从当前线程剥离开了。但是它并没有终止，而是交给其他线程去执行。具体交给谁，就看调度器怎么安排了。
...
所以，挂起就是一个：别管我，你干你的，我搞完了就回来。而这就是体现非阻塞的地方。我去别的地方执行任务，你不用管我，继续赶你当前的活。我那边搞完了就回来找你。这也是协程能讲阻塞代码协程非阻塞的原因。
